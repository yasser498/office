    ...
    <div className={`bg-white p-5 md:p-8 rounded-[2.5rem] shadow-2xl border transition-all ${editingReport ? 'border-indigo-400 ring-8 ring-indigo-50' : 'border-slate-100'}`}>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-2xl font-black text-slate-800">{editingReport ? 'تعديل التقرير' : 'إضافة إجراء جديد'}</h3>
            <p className="text-sm font-bold text-slate-400 mt-1">المستهدفون: {selectedEmployees.length} موظفاً</p>
          </div>
        </div>
        {editingReport && (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
          <div className="space-y-2 lg:col-span-4">
            <label className="text-sm font-black text-slate-700 mr-2">نوع الإجراء</label>
            <select
              value={formData.type}
              onChange={(e) => setFormData({ ...formData, type: e.target.value as ReportType })}
              className="w-full px-5 py-3.5 border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-100 transition-all bg-slate-50 font-black text-slate-700"
            >
            ...
            <>
              ...
              <div className="space-y-2 lg:col-span-2">
                ...
                <input
                  ...
                  className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-100 font-bold"
                  placeholder="مثلاً: 123/أ"
                />
              </div>
            </>
          )}
        ...
        <div className="space-y-2">
          <label className="text-sm font-black text-slate-700 mr-2">الأسباب / ملاحظات (تظهر في المحتوى)</label>
          <textarea
            rows={3}
            value={formData.notes}
            onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
            className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-[1.8rem] outline-none focus:ring-4 focus:ring-indigo-100 transition-all resize-none font-bold text-slate-700"
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[2rem] font-black text-xl transition-all shadow-2xl active:scale-95"
        >
          <Send size={24} />
          {isSubmitting ? 'جاري الحفظ...' : 'حفظ الإجراء وتوليد السجلات'}
        </button>
      </form>
    </div>
  );
